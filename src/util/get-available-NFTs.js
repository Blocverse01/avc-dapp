...
export async function getNFTs() {
  try {
    const nfts = await alchemy.nft.getNftsForOwner(import.meta.env.VITE_NFT_WALLET_ADDRESS);
...
export function getRandomTokens(tokenIds, number) {
  let _arr = [...tokenIds];
  return [...Array(number)].map(() => _arr.splice(Math.floor(Math.random() * _arr.length), 1)[0]);
}
